...
**The transport cost is `r scales::percent(tc$empirical_cost)`** as expected.
...
To save this result in `dit`, we use `save_result` = TRUE`.

Thus, **the differences-in-transports estimator is `r scales::percent(dit$empirical_cost)` at an optimal bandwidth of `r dit$optimal_bandwidth`**. 
...
dit <- diftrans(pre_main = pre_treated, post_main = post_treated,
                   ...
                   estimator = "dit", var = x,
                   bandwidth_vec = seq(0, 10, 1),
                   minimum_bandwidth = d_a) # smallest bandwidth will be d_a
dit$empirical_table
dit$empirical_cost
dit$optimal_bandwidth
```
...
**Accounting for sampling variability, our differences-in-transports estimate is still `r scales::percent(dit$empirical_cost)`**. 
...
Finally, we have that the **conservative differences-in-transports estimator is `r scales::percent(dit$empirical_cost)` at an optimal bandwidth of `r dit$optimal_bandwidth`**.
