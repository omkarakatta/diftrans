...
if (show_fig | show_fig8){
  ...
  realB_prop <- diftrans(B2010, B2011, bandwidth = bandwidth_seq,
                         conservative = T,
                         quietly = T)$main2d
  realT_prop <- diftrans(T2010, T2011, bandwidth = bandwidth_seq,
                         quietly = T)$main

  realdiff <- matrix(rep(realB_prop - realT_prop, numsim), nrow = numsim, byrow = T)

  diffprop <- placeboB_prop - placeboT_prop - realdiff

  bandwidth_selection <- data.frame(bandwidth = bandwidth_seq,
                                    diffprop = apply(diffprop, 2, mean)) %>%
    mutate(diffprop_lag = lag(diffprop, 1)) %>%
    mutate(diffprop_diff = abs(diffprop - diffprop_lag)) %>%
    mutate(closetozero = diffprop_diff < 1e-2) %>%
    mutate(closetozero_lag1 = lag(closetozero)) %>%
    mutate(closetozero_lag2 = lag(closetozero, 2)) %>%
    mutate(valid = closetozero*closetozero_lag1*closetozero_lag2)
...
if (show_fig | show_fig8){
  ...
  diffprop <- abs(placeboB_prop - placeboT_prop - realdiff)
